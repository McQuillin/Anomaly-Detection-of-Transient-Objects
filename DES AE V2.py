...
class LabelsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image = image.reshape(5,4,128)
        image_err = np.array([image_err.transpose()])
        #image_err = image_err.reshape(5,4,128)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = nn.Sequential(
            ...
            #nn.BatchNorm2d(3),
            nn.ReLU(),
            #nn.MaxPool2d((1,2),return_indices=True),
            )
        ...
        self.unpool2 = nn.MaxUnpool2d((1,2))
        
        
        self.decoder1 = nn.Sequential(            
            nn.ConvTranspose2d(8, 7, (1,41)),
            #nn.BatchNorm2d(7),
            nn.ReLU(),
            )
        self.decoder2 = nn.Sequential(            
            nn.ConvTranspose2d(7, 6, (1,32)),
            #nn.BatchNorm2d(6),
            nn.ReLU(),
            )
        self.decoder3 = nn.Sequential(            
            nn.ConvTranspose2d(6, 5, (1,32)),
            #nn.BatchNorm2d(5),
            nn.ReLU(),
            )
        self.decoder4 = nn.Sequential(            
            nn.ConvTranspose2d(5, 4, (2,16)),
            #nn.BatchNorm2d(4),
            nn.ReLU(),
            )
        self.decoder5 = nn.Sequential(    
            nn.ConvTranspose2d(4, 3, (2,8)),
            #nn.BatchNorm2d(3),
            nn.ReLU(),
            )
        self.decoder6 = nn.Sequential(    
            nn.ConvTranspose2d(3, 2, (2,4)),
            #nn.BatchNorm2d(2),
            nn.ReLU(),
            #nn.Dropout(0.5),
            #nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        x1 = self.encoder1(x) #, indices1
        x2  = self.encoder2(x1)#
        x3 = self.encoder3(x2)
        x4 = self.encoder4(x3)
        x5 = self.encoder5(x4)
        encoded = self.encoder6(x5)
        ...
        #unpooled = self.unpool1(unflat_encoded,indices6)#,output_size=(64,8,1,2))
        decoded1 = self.decoder1(unflat_encoded)
        
        #unpooled = self.unpool2(decoded1,indices5)
        decoded2 = self.decoder2(decoded1)
        
        #unpooled = self.unpool2(decoded2,indices4)
        decoded3 = self.decoder3(decoded2)
        
        #unpooled = self.unpool2(decoded3,indices3)
        decoded4 = self.decoder4(decoded3)
        
        #npooled = self.unpool2(decoded4,indices2)
        decoded5 = self.decoder5(decoded4)
        
        #unpooled = self.unpool2(decoded5,indices1,output_size=[64,3,3,127])
        decoded = self.decoder6(decoded5)
        
        
        
        print(unflat_encoded.size())
        print(decoded1.size())
        print(decoded2.size())
        print(decoded3.size())
        print(decoded4.size())
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.flat = nn.Flatten()
        self.unflat = nn.Unflatten(1, [2,1,1])
...
data  = LabelsDataset(csv_file='cutouts/info - copy.csv',
                                    root_dir='cutouts',
                                    transform=ToTensor())
...
optimizer = optim.Adam(net.parameters(), lr=0.01, weight_decay=5e-8)
...
acc_store = np.array([])
vacc_store = np.array([])
best_loss = best_vloss = 1000000000
"""
model = "model_3905"
optim = "op_model_3905"
...
for epoch in range(100000):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if(avg_loss < best_loss and 
           avg_loss < 1):
            best_loss = avg_loss
            
            I_in = inputs.detach().numpy()[0,0,:,:]
            I_in_err = inputs.detach().numpy()[0,1,:,:]
            
            
            I_out = outputs[0,0,:,:]
            I_out_err = outputs[0,1,:,:]
...
plt.plot(vacc_store)
plt.show()
np.savetxt("Models/model_info.csv", np.transpose([all_models,all_optims]),fmt="%s",delimiter=",",header="model_id,optim_id")
